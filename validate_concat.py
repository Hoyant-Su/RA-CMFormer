...
parser = argparse.ArgumentParser(description='Liver tumor differentiation Validation')
...
def validate(args):
    ...
    category_indices_info = dataset.category_indices
    feat_header = dataset.header
    ...
    if args.checkpoint:
        strict_type = True
        if not strict_type:
            print("Warning: the checkpoint loading protocal is not strict for now!")
            
        if 'uniformer' in args.model:
            checkpoint = torch.load(args.checkpoint, map_location='cpu')
            state_dict = checkpoint.get('state_dict', checkpoint)

            if 'feat_fc_layer.weight' in state_dict:
                feat_dim = state_dict['feat_fc_layer.weight'].shape[1]
                print(f"Extracted feature dim: {feat_dim}")

        load_checkpoint(model, args.checkpoint, args.use_ema, strict=strict_type)

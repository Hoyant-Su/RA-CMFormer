...
from models.RA_CMFormer import RA_CMFormer_
...
parser.add_argument('--feat_lens', default=-1,type=int)
...
parser.add_argument('--fine-tuned-checkpoint', default='', type=str, metavar='PATH',
                    help='Load fine-tuned model weights (full model, used before freezing backbone)')
...
parser.add_argument('--frozen-backbone', action='store_true', default=False,
                    help='Freeze backbone parameters, only train final_proj layer (for RU_Net_c_attn_gate_21)')
...
def main():
    ...
    category_indices_info = dataset_train.category_indices
    ...
    elif args.model == "RA_CMFormer":
        model = RA_CMFormer_(num_classes=args.num_classes)
        # Priority: fine-tuned checkpoint > initial checkpoint
        if args.fine_tuned_checkpoint:
            if args.local_rank == 0:
                _logger.info(f'Loading fine-tuned model weights from {args.fine_tuned_checkpoint}')
            fine_tuned_weights = torch.load(args.fine_tuned_checkpoint, map_location=torch.device('cpu'))
            # Handle both state_dict format and direct weights
            if isinstance(fine_tuned_weights, dict) and 'model' in fine_tuned_weights:
                fine_tuned_weights = fine_tuned_weights['model']
            elif isinstance(fine_tuned_weights, dict) and 'state_dict' in fine_tuned_weights:
                fine_tuned_weights = fine_tuned_weights['state_dict']
            ...
            pretrained_weights = torch.load(args.initial_checkpoint, map_location=torch.device('cpu'))
    ...
    if args.frozen_backbone and args.model == "RA_CMFormer":
        if args.local_rank == 0:
            _logger.info('Freezing backbone parameters, only training RadiomicsGuidedChannelAttention3D layers')
        # Freeze all parameters first
        for param in model.parameters():
            param.requires_grad = False
        
        # Unfreeze only channel attention layers (RadiomicsGuidedChannelAttention3D)
        if hasattr(model, 'channel_attn1'):
            for param in model.channel_attn1.parameters():
                param.requires_grad = True
        if hasattr(model, 'channel_attn2'):
            for param in model.channel_attn2.parameters():
                param.requires_grad = True
        if hasattr(model, 'channel_attn3'):
            for param in model.channel_attn3.parameters():
                param.requires_grad = True
        if hasattr(model, 'channel_attn4'):
            for param in model.channel_attn4.parameters():
                param.requires_grad = True
        ...
        if args.local_rank == 0:
            ...
            _logger.info(f'Frozen backbone: {trainable_params}/{total_params} ({trainable_pct:.2f}%) parameters are trainable')
            _logger.info('Trainable layers: channel_attn1, channel_attn2, channel_attn3, channel_attn4 (RadiomicsGuidedChannelAttention3D)')
    ...
    if args.opt == "muon":
        class MuonAdamHybrid(torch.optim.Optimizer):
            def __init__(self, model, lr=1e-3, muon_lr=1e-2, weight_decay=0.01, momentum=0.9, nesterov=True):
                linear, other = [], []
                for p in model.parameters(): linear.append(p) if p.ndim==2 else other.append(p)
                self.muon = torch.optim.Muon(linear, lr=muon_lr, weight_decay=weight_decay, momentum=momentum, nesterov=nesterov)
                self.adamw = torch.optim.AdamW(other, lr=lr, weight_decay=weight_decay)
                param_groups = self.muon.param_groups + self.adamw.param_groups
                super(MuonAdamHybrid, self).__init__(param_groups, {})
                self._muon_param_ids = {id(p) for group in self.muon.param_groups for p in group['params']}
                self._adamw_param_ids = {id(p) for group in self.adamw.param_groups for p in group['params']}
            ...
            def state_dict(self):
                ...
                if 'state' in muon_state:
                    combined_state['state'].update(muon_state['state'])

                if 'state' in adamw_state:
                    combined_state['state'].update(adamw_state['state'])
                
                return combined_state
            
            def load_state_dict(self, state_dict):
                muon_state = {'state': {}, 'param_groups': self.muon.param_groups}
                adamw_state = {'state': {}, 'param_groups': self.adamw.param_groups}
                    ...
                    for param_id, state in state_dict['state'].items():
                        ...
                        for group in self.muon.param_groups:
                            for p in group['params']:
                                if id(p) == param_id:
                                    muon_state['state'][param_id] = state
                                    found_in_muon = True
                                    break
                            if found_in_muon:
                                break
                        
                        if not found_in_muon:
                            for group in self.adamw.param_groups:
                                for p in group['params']:
                                    if id(p) == param_id:
                                        adamw_state['state'][param_id] = state
                                        break

                self.muon.load_state_dict(muon_state)
                self.adamw.load_state_dict(adamw_state)
        
        optimizer = MuonAdamHybrid(model, lr=args.lr, muon_lr=0.02, weight_decay=0.01, momentum=args.momentum)
    else:
    ...
    if args.smoothing:
        ...
        #train_loss_fn = FocalLoss(gamma=2).cuda()
    ...
    try:
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            if metric_savers is not None:
                ...
                for metric in args.report_metrics:
                    #print("eval_metrics:",eval_metrics[metric])
                    #print("best_metrics:",best_metrics[metric]['value'])


                    #if best_metrics[metric]['value'] is None or (np.mean(eval_metrics[metric]) > best_metrics[metric]['value']): 
                    if best_metrics[metric]['value'] is None or (eval_metrics[metric] > best_metrics[metric]['value']): 
                        #best_metrics[metric]['value'] = np.mean(eval_metrics[metric])
